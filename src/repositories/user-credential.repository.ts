...
export class UserCredentialRepository implements IUserCredentialRepository{
    ...
    async findByEmailAndPassword(email: string, password: string): Promise<UserCredential> {
        return await this.manager.createQueryBuilder()
            .select()
            .from(UserCredential, 'user_credential')
            .where('user_credential.email = :email AND user_credential.password = :password', {email, password})
            .getOne();
    }
